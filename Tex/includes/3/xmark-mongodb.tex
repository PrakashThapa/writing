...
\paragraph{Indexing} 
	Each document is Mongodb is uniquely identified by a field "\_id" which is a primary key. Hence the collection is sorted by \_id by default. \todo{but note that this primary key index is not a clustered index in Database terms. I.e. the index entries only contains pointers to actual documents in the MongoDB data files. Documents are not physically stored in the order of \_id on disks.}~\cite{nosql/comparision} ::expand...page:9

\subsubsubsection{XMARK}
